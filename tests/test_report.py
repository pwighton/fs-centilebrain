...
def test_html_contains_table_values_and_flags():
    ...
    assert "Normative neuromorphometry report" in html and "not been approved by the FDA" in html
...
def test_render_report_from_json_only(tmp_path):
    ...
    assert pdf == tmp_path / "normative-neuromorphometry-report--subject-s.pdf"
    assert pdf.read_bytes()[:5] == b"%PDF-"
    assert pdf.stat().st_size > 50_000                      # includes the embedded PNGs
    html = (tmp_path / "output" / "normative-neuromorphometry-report--subject-s.html").read_text()
